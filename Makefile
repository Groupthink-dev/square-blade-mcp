...
help:
	...
	@echo "  make test-e2e         - Run e2e tests (requires live Square sandbox)"
...
test-e2e:
	SQUARE_E2E=1 uv run pytest tests/ -v -m e2e

test-cov:
	uv run pytest tests/ -m "not e2e" --cov=src/square_blade_mcp --cov-report=term-missing -v
...
type-check:
	uv run mypy src/square_blade_mcp
